...
get_gene_annotation <- function(gene_annot_file_name, chrom, gene_types=c('protein_coding', 'pseudogene', 'lncRNA')){
...
get_cis_genotype <- function(gt_df, snp_annot, coords, cis_window) {
  snp_info <- snp_annot %>% filter((pos >= (coords[1] - cis_window) & !is.na(rsid_dbSNP151)) & (pos <= (coords[2] + cis_window)))
  if (nrow(snp_info) == 0)
    return(NA)
...
get_covariates <- function(covariate_file_name, samples) {
  cov_df <- read.table(covariate_file_name, header = TRUE, stringsAsFactors = TRUE, row.names = 1)
  rownames(cov_df)[grepl('\\d*-\\d*',rownames(cov_df))] <-
    paste('X',rownames(cov_df)[grepl('\\d*-\\d*',rownames(cov_df))],sep='')
  rownames(cov_df) <- gsub('-','.',rownames(cov_df))
  cov_df <- cov_df[samples,] %>% as.data.frame()
  cov_df
}
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  # Prepare output data----
  model_summary_file <- 'path_to_summary_folder/' %&% prefix %&% '_chr' %&% chrom %&% '_model_summaries.txt'
  ...
  weights_file <- 'path_to_weights_folder/' %&% prefix %&% '_chr' %&% chrom %&% '_weights.txt'
  weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
  write(weights_col, file = weights_file, ncol = 6, sep = '\t')

  tiss_chr_summ_f <- 'path_to_summary_folder/' %&% prefix %&% '_chr' %&% chrom %&% '_tiss_chr_summary.txt'
  ...
  covariance_file <- 'path_to_covariances_folder/' %&% prefix %&% '_chr' %&% chrom %&% '_covariances.txt'
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          ...
          weighted_snps_info <- snp_annot %>% filter(varID %in% weighted_snps) %>% select(rsid_dbSNP151, varID, ref_vcf, alt_vcf)
          ...
          weighted_snps_info <- weighted_snps_info %>%
            ...
            select(gene, rsid_dbSNP151, varID, ref_vcf, alt_vcf, weights)
          write.table(weighted_snps_info, file = weights_file, append = TRUE, quote = FALSE, col.names = FALSE, row.names = FALSE, sep = '\t')
          covariance_df <- do_covariance(gene, cis_gt, weighted_snps_info$rsid_dbSNP151, weighted_snps_info$varID)
